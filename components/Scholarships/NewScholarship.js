...
class NewScholarship extends React.Component {
  state = {
    title: "",
    arTitle: "",
    titleError: "",
  ...
  validateNumber = () => {
    if (this.state.fees !== "") {
      if (isNaN(parseInt(this.state.fees)))
        this.setState({
          feesError: "You should Enter a valid number!!",
        });
    }
  };
  ...
  handleSave = async () => {
    if (this.validateInput()) {
      const result = await AdminServices.scholarshipsServices.createNewScholarship(
        this.state.title,
        this.state.arTitle,
        this.state.fees,
...
export default withMyHook(NewScholarship);
